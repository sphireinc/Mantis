...
func TestParseBytesError(t *testing.T) {
	// int out of range
	ioor := "100000000000000000000000000"
	// float out of range
	foor := "1.0e512"
	tests := []struct {
		expected error
		...
		{fmt.Errorf(`units.ParseBytes: parsing "10 AiB": unknown bytes unit`), "10 AiB"},
		{fmt.Errorf(`units.ParseBytes: parsing "asdf": bad string format`), "asdf"},
		{fmt.Errorf(`units.ParseBytes: parsing "` + ioor + ` PiB": strconv.ParseInt: parsing "` + ioor + `": value out of range`), ioor + " PiB"},
		{fmt.Errorf(`units.ParseBytes: parsing "` + foor + ` PiB": strconv.ParseFloat: parsing "` + foor + `": value out of range`), foor + " PiB"},
	}
	for i, test := range tests {
		t.Run(fmt.Sprintf("%d", i), func(t *testing.T) {
			_, err := ParseBytes(test.parse)
			if err == nil {
